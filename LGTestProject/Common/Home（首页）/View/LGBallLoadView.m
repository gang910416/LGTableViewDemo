...
typedef NS_ENUM(NSUInteger, LGBallMoveDirection){
    LGBallMoveDirectionPositive = 1,    // 正向运动
    LGBallMoveDirectionNegative = -1    // 逆向运动
};

@interface LGBallLoadView ()
// 球容器视图
...
@property (nonatomic, assign) LGBallMoveDirection   moveDirection;
...
@implementation LGBallLoadView

+ (instancetype)loadingViewInView:(UIView *)view {
    LGBallLoadView *loadingView = [[LGBallLoadView alloc] initWithFrame:view.bounds];
...
- (void)setupUI {
    ...
    // 方向
    self.moveDirection = LGBallMoveDirectionPositive;
...
- (void)updateBallAnimations {
    if (self.moveDirection == LGBallMoveDirectionPositive) { // 正向
        ...
        if (CGRectGetMaxX(self.greenBall.frame) >= self.containerView.bounds.size.width || CGRectGetMinX(self.redBall.frame) <= 0) {
            // 切换为反向
            self.moveDirection = LGBallMoveDirectionNegative;
    ...
    }else if (self.moveDirection == LGBallMoveDirectionNegative) { // 反向
        // 更新绿球位置
        CGPoint center = self.greenBall.center;
        center.x -= kBallSpeed;
        self.greenBall.center = center;
